...
def get_web3_instance() -> Tuple[Web3, str]:
    """
    Returns a configured Web3 instance and RPC URL, or raises
...
def get_contract_address() -> str:
    addr = os.getenv("CONTRACT_ADDRESS") or os.getenv(
        "BLUE_CARBON_CREDIT_ADDRESS") or DEFAULT_CONTRACT_ADDRESS
    return Web3.to_checksum_address(addr.strip())


def get_verifier_account(w3: Web3):
    pk = (os.getenv("VERIFIER_PRIVATE_KEY")
          or os.getenv("PRIVATE_KEY") or "").strip()
    if not pk:
        raise BlockchainConfigError(
            "Blockchain verifier wallet is not configured.")
    ...
    try:
        ...
        raise BlockchainConfigError(
            f"Invalid verifier private key: {e}") from e
...
def check_verifier_role() -> Dict[str, Any]:
    ...
    try:
        ...
        has_role = contract.functions.hasRole(
            VERIFIER_ROLE, account.address).call()
        role_error = (
            None
            if has_role
            else (
                f"Verifier wallet {account.address} does not have required "
                f"VERIFIER_ROLE on contract {contract_addr}"
            )
        )
        return {
            "configured": True,
...
def register_submission_onchain(
    ...
    if not w3.is_connected():
        raise BlockchainExecutionError(
            f"Unable to connect to Sepolia RPC endpoint at {rpc_url[:25]}...")
    ...
    if not Web3.is_address(beneficiary_address):
        raise BlockchainExecutionError(
            f"Invalid beneficiary wallet address: {beneficiary_address}")
    ...
    try:
        ...
        raise BlockchainExecutionError(
            f"Failed to query verifier role: {e}") from e
    ...
    try:
        ...
        raise BlockchainExecutionError(
            f"Smart contract transaction failed: {e}") from e
