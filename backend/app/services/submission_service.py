...
class SubmissionService:
    ...
    async def _rest(self, method: str, path: str, **kwargs: Any) -> Any:
        ...
        if response.status_code >= 400:
            ...
            raise SubmissionServiceError(err_msg)
        return response.json() if response.content else None

    async def _pin_evidence(self, evidence: dict[str, Any]) -> str:
        if not self.pinata_jwt:
            raise SubmissionServiceError(
                "PINATA_JWT must be configured on the backend."
            )
        async with httpx.AsyncClient(timeout=45) as client:
            response = await client.post(
                ...
                json={
                    ...
                    "pinataMetadata": {"name": "blue-carbon-evidence.json"},
                },
            )
        if response.status_code >= 400:
            raise SubmissionServiceError(
                f"Pinata upload failed with HTTP {response.status_code}."
            )
    ...
    async def create_submission(
        ...
        evidence = {
            ...
            "photo_data_url": submission.photo_data_url,
            "scoring": scoring,
        }
        ...
        row = {
            ...
            "longitude": submission.longitude,
            "accuracy": submission.accuracy,
            "planted_date": submission.claimed_planting_date.isoformat(),
            "photo_url": f"ipfs://{cid}",
            "score": scoring["score"],
            "ndvi_before": scoring["ndvi_before"],
            "ndvi_after": scoring["ndvi_after"],
        ...
        res = await self._rest(
            "POST", "submissions", params={"select": "*"}, json=row
        )
        saved = res[0]
        flagged = bool(scoring["flags"])
        log_text = (
            f"{submission.project_name} "
            f"{'flagged' if flagged else 'scored'} — "
            f"NDVI score {scoring['score']}"
        )
        await self._rest(
            "POST",
            "activity_log",
            json={
                "kind": "flag" if flagged else "submit",
                "text": log_text,
        ...
        is_eligible = (
            scoring["score"] >= 75
            and scoring["confidence_band"] == "high"
            and not flagged
        )
        return {
            "submission": saved,
            "ipfs_cid": cid,
            "eligible_for_provisional": is_eligible,
        }

    async def list_submissions(self) -> list[dict[str, Any]]:
        return await self._rest(
            "GET", "submissions", params={"select": "*", "order": "created_at.desc"}
        )
    ...
    async def list_activity(self, limit: int) -> list[dict[str, Any]]:
        ...
        return await self._rest("GET", "activity_log", params=params)

    async def review(self, submission_id: str,
                     review: ReviewRequest) -> dict[str, Any]:
        final_status = review.action
        if review.action == "resolved":
            if review.approved is None:
                raise SubmissionServiceError(
                    "A resolved dispute requires approved=true or approved=false.")
        ...
        if review.tx_hash:
            patch["on_chain_tx"] = review.tx_hash
        if review.block_number is not None:
            patch["on_chain_block"] = review.block_number
        ...
        await self._rest(
            ...
            json={
                "kind": kind,
                "text": f"Submission {submission_id[:8]}… {final_status}",
    ...
    def _detect_region(lat: float, lng: float) -> str:
        if 21 < lat < 23 and 87 < lng < 90:
            return "West Bengal"
        if 22 < lat < 25 and 84 < lng < 87:
            return "Jharkhand"
        if 20 < lat < 24 and 68 < lng < 72:
            return "Gujarat"
        if 8 < lat < 13 and 74 < lng < 78:
            return "Kerala"
